...
GLuint Shader::CreateShader(const std::string& vertexSource, const std::string& fragmentSource)
{
	...
	GL_CALL(glAttachShader(program, vs));	// 绑定顶点着色器
	GL_CALL(glAttachShader(program, fs));	// 绑定片段着色器
	GL_CALL(glLinkProgram(program));			// 链接程序，将所有着色器合并为一个可执行的程序
	GL_CALL(glValidateProgram(program));		// 验证程序对象是否可以在当前的 OpenGL 状态下执行

	GL_CALL(glDeleteShader(fs));				// 删除着色器对象 因为一旦着色器被链接到程序对象，着色器的代码已经被链接到程序中，所以可以安全地删除着色器对象
