...
class FPathFindAction final : public FPendingLatentAction
{
	struct FDelegateHelper final : FGCObject
	{
		...
		explicit FDelegateHelper(UNavigationPath* NavPath, AActor* GoalActor, const float TetherDistance):
			GoalActor(GoalActor),
			Path(NavPath),
		...
		void PathfindingFinished(const uint32 QueryID, const ENavigationQueryResult::Type Result, const FNavPathSharedPtr NavPath)
		{
			if (Path)
			{
				...
				if (Path->GetPath())
				{
					...
					if ((Result == ENavigationQueryResult::Success || (NavPath && NavPath->IsPartial())) && GoalActor.IsValid())
					{
...
public:
	...
	{
		return FString::Printf(TEXT("FPathFindAction finished: %s"), Delegate.Get().bFinished ? TEXT("True") : TEXT("False"));
	}
#endif
};
...
TArray<FVector> UCatmullRomSpline::EquidistantSamples(const float SampleLength) const
{
	...
	TArray<FVector> SamplePoints; SamplePoints.Reserve(NumSegments + 1);
	...
	const bool bPersistent)
{
	...
	if (NavData)
	{
		...
		if (NumPathVerts > 0)
		{
			DrawDebugBox(World, PathPoints[NumPathVerts - 1].Location + PathOffset, FVector(15.f), PathColor, bPersistent);
		}
	...
	const TArray<FVector>& PathPoints)
{
	...
	if (DefaultNavData != nullptr && OldPath != nullptr)
	{
		...
		FNavPathSharedPtr NewPath = NavSys->GetDefaultNavDataInstance()->CreatePathInstance<FNavigationPath>(OldPath->GetQueryData());
	...
	EPathFindingMode::Type Mode)
{
	...
	const AFlyingNavigationData* FlyingNavData = Cast<AFlyingNavigationData>(Query.NavData.Get());

	if (FlyingNavData)
	{
		const uint32 QueryID = NavSys->FindPathAsync(AgentProperties, Query, ResultDelegate, Mode);
...
void UFlyingNavFunctionLibrary::AbortAsyncFindPathRequest(
	UNavigationSystemV1* NavSys,
	...
	const TSubclassOf<UNavigationQueryFilter> FilterClass)
{
	...
	if (NavSys != nullptr && NavSys->GetDefaultNavDataInstance() != nullptr)
	{
		...
		const bool bUsingFlyingNavigation = (FlyingNavData != nullptr);
		...
		if (bUsingFlyingNavigation)
		{
			Query.bAllowPartialPaths = false;
		}
		
		// Create NavPath early, to set GoalPawn
		if (GoalActor && GoalActor->IsA(APawn::StaticClass()) && bUsingFlyingNavigation)
		{
		...
		FPathFindAction* PathFindAction = new FPathFindAction(ResultPath, ResultDelegate, LatentInfo, GoalActor, TetherDistance);
		const FAsyncPathQueryHandle QueryHandle = FindPathAsync(NavSys, FNavAgentProperties::DefaultProperties, Query, ResultDelegate);
		...
		} else
		{
			...
			printw("WARNING: INVALID_NAVQUERYID")
	...
	} else
	{
		if (NavSys == nullptr)
		{
			printw("WARNING: NavSys == nullptr.")
		} else
		{
			printw("WARNING: NavSys->GetDefaultNavDataInstance() == nullptr. Check a valid default agent is set in Project Settings and a Nav Mesh Bounds Volume is present in the level.")
		}
...
UNavigationPath* UFlyingNavFunctionLibrary::FindPathToActorAsynchronously(
	...
	const FLatentActionInfo LatentInfo,
	const FVector& PathStart, AActor* GoalActor,
	float TetherDistance,
	...
	TSubclassOf<UNavigationQueryFilter> FilterClass)
{
	...
	{
		printw("WARNING: Can't find path to null actor")
		return nullptr;
	}
...
bool UFlyingNavFunctionLibrary::IsPositionAValidEndpoint(const APawn* NavAgent, const FVector& Position, const bool bAllowBlocked)
{
	...
	{
		printw("WARNING: NavAgent doesn't use Flying Navigation")
		return false;
	}
...
void UFlyingNavFunctionLibrary::RequestMove(UNavigationPath* PathToFollow, AAIController* Controller)
{
	...
	if (Controller && PathFollowingComponent && PathToFollow && PathToFollow->IsValid())
	{
		...
		PathFollowingComponent->RequestMove(MoveReq, PathToFollow->GetPath());
	} else
	{
		printw("WARNING: Valid Controller: %s, Valid PathToFollow: %s",
...
void UFlyingNavFunctionLibrary::SmoothPathPoints(TArray<FNavPathPoint>& NavPathPoints, const float SampleLength)
{
	// Convert from TArray<FNavPathPoint> to TArray<FVector>
	TArray<FVector> PathPoints;
